...
export class ClineProvider implements vscode.WebviewViewProvider {
	public static readonly sideBarId = "bouga-lms-ai.SidebarProvider" // used in package.json as the view's id. This value cannot be changed due to how vscode caches views based on their id, and updating the id would break existing instances of the extension.
	public static readonly tabPanelId = "bouga-lms-ai.TabPanelProvider"
				...
				switch (message.type) {
					...
					case "openExtensionSettings": {
						...
						await vscode.commands.executeCommand(
							...
							`@ext:bouga.bouga-lms-ai ${settingsFilter}`.trim(), // trim whitespace if no settings filter
						)
						break
					}
					case "invoke": {
	...
	async fetchUserCreditsData() {
		try {
			await Promise.all([
				...
				this.bougaLMSApiService?.fetchBalance(),
